...
(function (window, $) {
    ...
    $.fn.cardpic = function (options) {
        ...
        return this.each(function () {
            var opts = $.extend({}, $.fn.cardpic.defaults, options);
            ...
            //初始化函数
            function init() {
                $this.addClass('pi-cardpic').html('<div class="pi-cardpic-wrap">' + $this.html() + '</div>' + (isShowTitle ? '<div class="pi-cardpic-title"></div>' : ''));

                $wrap = $this.find('.pi-cardpic-wrap');
                wrapElStyle = $wrap[0].style;
                $items = $wrap.children('*');
                itemCount = $items.length;

                isVertical && $this.addClass('vertical');
                $title = $this.find('.pi-cardpic-title');

                //pager
                var html = '';
                if (isShowPager) {
                    html += '<div class="pi-cardpic-pager">';
                    for (var i = 0, len = itemCount; i < len; i++) {
                        html += '<span></span>';
                    }
                    html += '</div>';
                }
                $pagers = $this.append(html).find('.pi-cardpic-pager span');
